...
@section('content')
                            ...
                            <tbody>
                                ...
                                @foreach ($bookings as $b)
                                    <tr class='text-capitalize'>
                                        <td>{{ $no }}</td>
                                        <?php $no++ ?>
                                        <td {!!$b->status_time !!}>{{ $b->name }}</td>
                                        <td {!!$b->status_time !!}>{{$b->user->name}}</td>
                                        <td {!!$b->status_time !!}>{{$b->packet->nama}}</td>
                                        <td {!!$b->status_time !!}>{{$b->no_nota}}</td>
                                        <td {!!$b->status_time !!}>{{$b->formatted_date}}</td>
                                        <td>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <div id="tampil-cancel" class="table-responsive mt-5">
                        {{-- bookingTable --}}
                        <table class="bookingTable table">
                            <thead>
                                <tr>
    ...
    <div class="modal fade" id="modal-del-list" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form method="POST" action="{{ route('report-partner.cancel') }}" enctype="multipart/form-data">
                    @method('post')
                    @csrf
    ...
    <div class="modal fade" id="modal-report" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('report-partner.update') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('post')
                    <div class="modal-body">
                        <div class="form-row">
    ...
    <script>
        ...
        <script>
                const BASE_DELETE_URL = '{{ route('report-partner.cancel') }}';
        ...
        $('.pilih').click(function() {
            ...
            var year = $("#yearNumber").val()
                ...
                $.ajax({
                    ...
                    type:"GET",
                    url:"/monthsortingdpm/"+month+"/"+year,
                    success: function(data) {
                        var myhtml = "";
                            myhtml += "<table class='bookingTableweek table'><thead><tr><th scope='col'>No</th><th scope='col'>Nama Pemesan</th><th scope='col'>Nama Mitra</th><th scope='col'>Nama Paket</th>"
                            myhtml += "<th scope='col'>No Nota</th><th scope='col'>No Whatsapp</th><th scope='col'>Tanggal Potong</th><th scope='col'>Status Proses</th></tr></thead><tbody>"
