...
global $wpdb;
$mitra = $_GET['mitra'];
$paket = $wpdb->get_results("SELECT * FROM ugi_akikahkita_paket JOIN backend_users ON ugi_akikahkita_paket.id_mitra_id = backend_users.id WHERE id_mitra_id=$mitra");
$mitra = $wpdb->get_results("SELECT * FROM backend_users WHERE role_id = '4'");
...
<div class="container py-3">
    ...
    <ul class="nav nav-pills mb-3 text-center" id="pills-tab" role="tablist">
        ...
        <li class="nav-item">
            <a class="mr-4" style="color:blue" id="pills-mitra-tab" data-toggle="pill" href="#pills-mitra" role="tab" aria-controls="pills-mitra" aria-selected="false">Mitra Lainnya</a>
        </li>
    </ul>
                ...
                <div class="row">
                    <?php foreach ($paket as $value) { ?>
                        <div class="col-md-3"> 
                            <div class="card mb-4">
                                <div class=" card-body">
                                    <h5 class="card-title"><strong><?= $value->nama_paket ?></strong></h5><br>
                                    <h6 class="card-subtitle mb-2 text-muted">Rp. <?= number_format($value->harga, 0, ',', '.') ?></h6>
                                    <p class="card-text"><?= $value->keterangan ?></p>
                                    <h6 class="card-subtitle mb-2 text-muted"><?= $value->kategori_paket ?></h6>
                                    <p class="card-text"><b>Mitra</b> : <?= $value->first_name ?></p>
                                </div>
                                <div class="card-footer" style="background: linear-gradient( rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5) ), url(<?php echo get_stylesheet_directory_uri() . '/image/1.png' ?>); background-size: 100%; 100%;">
                                    ...
                                    <a href="localhost:8000/formpesan/<?= $value->id ?>">Bli</a>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                        ...
                        <div class="col-md-3">
                            <div class="card mb-4">
                                <div class=" card-body">
                                    ...
                                    <h5 class="card-title text-center"><strong><?= $value->first_name ?></strong></h5>
                                    <p class="card-text">
                                ...
                                <div class="card-footer" style="background: linear-gradient( rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5) ), url(<?php echo get_stylesheet_directory_uri() . '/image/2.png' ?>); background-size: 100%; 100%;">
                                    <button class="btn btn-success float-right">Lihat Paket</button>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
